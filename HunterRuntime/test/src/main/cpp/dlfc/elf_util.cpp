...
ElfImg::ElfImg(const char *elf) {
    ...
    if (!symtab_offset) {
        LOGE("can't find symtab from sections %s ", elf);
    }
    ...
    if (dynsym_start != nullptr && strtab_start != nullptr) {
        ...
        for (k = 0; k < dynsym_count; k++, sym++)
            if (strcmp(strings + sym->st_name, name) == 0) {
                ...
                LOGI("ElfImg GetSymOffset find %s: %x\n", elf, _offset);
                return _offset;
            }
    }
...
//根据地址获取符号信息
char* ElfImg::getAddressSym(long address) {
    if (baseInRam != nullptr) {

        //参考上面的方法绝对地址的获取相对偏移
        long reAddress = (address - ((size_t) baseInRam)) + bias;
        LOGD("函数绝对地址 %lu  经过计算的偏移函数地址  %lu  ",address,reAddress);
        ...
        if (dynsym_start != nullptr && strtab_start != nullptr) {
            ...
            for (k = 0; k < dynsym_count; k++, sym++) {
                ...
                LOGD("相对开始地址  address  %llu ,函数长度 %d ",_offset,_size)

                if (strcmp(strings + sym->st_name, "open") == 0) {
                    LOGD("发现open ")
                }
                //判断是否在地址范围之内
//                if (reAddress >= _offset && reAddress <= (_offset + _size)) {
//                    //返回相对偏移
...
void *ElfImg::getModuleBase(const char *name) {
    ...
    //LOGD("ElfImg Get module baseInRam %s: %lu", name, load_addr);

    return reinterpret_cast<void *>(load_addr);
}

bool zhenxiisRXPerm(const char *perm) {
...
void ElfImg::searchMaps() {
    FILE *f;
    //if ((f = fopen("/proc/self/abcd", "r")) == NULL) {
