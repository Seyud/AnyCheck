...
public final class RposedHelpers {
    ...
    public static Class<?> findClass(String className, ClassLoader classLoader) {
        if (classLoader == null) {
            classLoader = RposedBridge.BOOTCLASSLOADER;
        }
        try {
    ...
    /**
     * hook all method for class
     */
    @SuppressWarnings("SameParameterValue")
    public static void HookClassAll(@NonNull Class<?> clazz,RC_MethodHook callback) {
//        new RC_MethodHook() {
//            @Override
//            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
//                super.beforeHookedMethod(param);
//                CLog.e("HookClassAll-> "+clazz.getName() + " " + method.getName() + " " + Arrays.toString(param.args));
//            }
//        }
        Method[] declaredMethods = clazz.getDeclaredMethods();
        for (var method : declaredMethods) {
            RposedBridge.hookAllMethods(clazz,method.getName(),callback);
        }
    }
     ...
     * for details.
     */
    public static RC_MethodHook.Unhook findAndHookMethod(Class<?> clazz, String methodName, Object... parameterTypesAndCallback) {
        if (parameterTypesAndCallback.length == 0 || !(parameterTypesAndCallback[parameterTypesAndCallback.length-1] instanceof RC_MethodHook)) {
            throw new IllegalArgumentException("no callback defined");
        }

        RC_MethodHook callback = (RC_MethodHook) parameterTypesAndCallback[parameterTypesAndCallback.length-1];
        Method m = findMethodExact(clazz, methodName, getParameterClasses(clazz.getClassLoader(), parameterTypesAndCallback));

        return RposedBridge.hookMethod(m, callback);
    }
     ...
     * an anonymous subclass of {@link RC_MethodHook} or {@link RC_MethodReplacement}.
     *
     * <p><b>Example</b>
     ...
     */
    public static RC_MethodHook.Unhook findAndHookMethod(String className, ClassLoader classLoader, String methodName, Object... parameterTypesAndCallback) {
    ...
    private static Class<?>[] getParameterClasses(ClassLoader classLoader, Object[] parameterTypesAndCallback) {
        ...
        for (int i = parameterTypesAndCallback.length - 1; i >= 0; i--) {
            ...
            if (type instanceof RC_MethodHook) {
                continue;
            }
     ...
     * for details.
     */
    public static RC_MethodHook.Unhook findAndHookConstructor(Class<?> clazz, Object... parameterTypesAndCallback) {
        if (parameterTypesAndCallback.length == 0 || !(parameterTypesAndCallback[parameterTypesAndCallback.length-1] instanceof RC_MethodHook)) {
            throw new IllegalArgumentException("no callback defined");
        }
     ...
     * for details.
     */
    public static RC_MethodHook.Unhook findAndHookConstructor(String className, ClassLoader classLoader, Object... parameterTypesAndCallback) {
