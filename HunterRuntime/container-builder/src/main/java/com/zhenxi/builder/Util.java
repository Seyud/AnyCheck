...
public class Util {
    ...
    public static void copyLibrary(ZipMaker zos, Set<String> supportArch, File apkFile) throws IOException {
    ...
    public static Set<String> zhenxiSupportArch = new HashSet<>();

    public static void setupzhenxiSupportArch(String settings) {
        zhenxiSupportArch.addAll(Arrays.asList(settings.split(",")));
        if (zhenxiSupportArch.contains("armeabi-v7a")) {
            zhenxiSupportArch.add("armeabi");
        }
    }

    static boolean iszhenxiUnSupportArch(String zipEntryName) {
        ...
        return !zhenxiSupportArch.contains(nowArch);
    }
...
//        return !ratelSupportArch.contains(nowArch);
    ...
    public static void copyAssets(ZipMaker zos, File from, String name) throws IOException {
        zos.putNextEntry("assets/" + name);
        zos.write(FileUtils.readFileToByteArray(from));
    }

    static final Pattern classesIndexPattern = Pattern.compile("classes(\\d+)\\.dex");
