...
import static org.jf.pxb.android.axml.NodeVisitor.TYPE_INT_BOOLEAN;
import static org.jf.pxb.android.axml.NodeVisitor.TYPE_STRING;

import org.jf.pxb.android.ResConst;
    ...
    public int next() throws IOException {
        if (fileSize < 0) {
            ...
            if (type != RES_XML_TYPE) {
                //apk的xml文件可能被混淆或者魔改,暂不做头部魔教判断
                System.out.println(">>>>>>>> apk xml file maybe proguard");
        ...
        int event = -1;
        for (int p = ((Buffer)in).position(); p < fileSize; p = ((Buffer)in).position()) {
            int type = in.getInt() & 0xFFFF;
            int size = in.getInt();
            switch (type) {
