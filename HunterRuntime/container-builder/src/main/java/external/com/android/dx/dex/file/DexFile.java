...
public final class DexFile {
    ...
    public DexFile(final DexOptions dexOptions) {
        ...
        /*
         * init the list of sections in the order they appear in
         * the final output.
         */
    ...
    private ByteArrayAnnotatedOutput toDex0(boolean annotate,
        ...
        wordData.prepare();
        if (dexOptions.apiIsSupported(DexFormat.API_METHOD_HANDLES)) {
            // init call site ids before byteData where the call site items are placed.
            callSiteIds.prepare();
        }
        byteData.prepare();
        if (dexOptions.apiIsSupported(DexFormat.API_METHOD_HANDLES)) {
            // init method handles after call site items placed in byteData.
